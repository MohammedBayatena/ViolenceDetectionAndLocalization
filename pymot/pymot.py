...
class MOTEvaluation:
    ...
    def get_hypotheses_frame(self, timestamp):
        ...
        #     raise Exception(
        #         "> 1 hypotheses timestamps found for timestamp %f with sync delta %f" % (timestamp, self.sync_delta_))
...
#                print "Correspondence: %s-%s" % (gt_id, hypo_id)
        ...
        for groundtruth in groundtruths:
            ...
            if groundtruth["id"] not in list(correspondences.keys()) and groundtruth.get("dco", False) != True:
...
#            raise("No correspondence found. MOTP calculation not possible")
    ...
    def printLegacyFormat(self):
        ...
        print(("DIFF TOTALS: groundT %.0f\t miss %.0f\t falseP %.0f\t mismatch %0.f" % (
        self.total_groundtruths_, self.misses_, self.false_positives_, self.mismatches_)))
        print((
            "ABS TOTALS: groundT %.0f\t corr %.0f\t failedCorr %.0f\t overlap_ratio %.2f\t miss %.0f\t falseP %.0f\t mismatch %0.f" % (
            self.total_groundtruths_, 0, 0, 0, self.misses_, self.false_positives_, self.mismatches_)))
